...
timing_font = pygame.font.Font("Assets/Fonts/PressStart2P.ttf", 25)
...
def draw_stats():
    ...
    if "towel" not in interacted_items:
        screen.blit(towel_img, towel_rect)
    if "mirror" not in interacted_items:
...
def get_game_time():
    ...
    return f"{hour_12:02}:{minutes:02} {suffix}"
...
def run():
    ...
    global mirror_click_count, game_minutes, last_minute_update
    ...
    while running:
        ...
        if now - last_minute_update > 1000:  # 1 second has passed
            game_minutes = (game_minutes + minutes_per_real_second) % (24 * 60)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    if menu_button_rect.collidepoint(event.pos):
                        ...
                        if slept_once:
                            ...
                            error_click.play()
                        elif not {"brush", "towel", "mirror"}.issubset(interacted_items):
                            # Not done all routines yet, show popup
                            interacted_items_popup = True
                            popup_timer = current_time
                            error_click.play()
                        else:
                            # All routines done, can sleep
                            sleeping = True
